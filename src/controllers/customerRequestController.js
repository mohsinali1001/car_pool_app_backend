...
const updateCustomerRequest = async (req, res) => {
  ...
  try {
    ...
    if (['accepted', 'completed'].includes(status)) {
      return res.status(400).json({ success: false, error: 'Accepted or completed requests cannot be edited', code: 'REQUEST_LOCKED' });
    }

    if (
      startLocation != null && exceedsMaxLength(startLocation, MAX_LOCATION) ||
      endLocation != null && exceedsMaxLength(endLocation, MAX_LOCATION) ||
      pickupLocation != null && exceedsMaxLength(pickupLocation, MAX_LOCATION) ||
      dropLocation != null && exceedsMaxLength(dropLocation, MAX_LOCATION)
    ) {
      return res.status(400).json({ success: false, error: `Location fields must be at most ${MAX_LOCATION} characters`, code: 'FIELD_TOO_LONG' });
    }

    const nextStartLocation = startLocation != null ? sanitizeString(labelFromLocation(startLocation), MAX_LOCATION) : request.startLocation;
    const nextEndLocation = endLocation != null ? sanitizeString(labelFromLocation(endLocation), MAX_LOCATION) : request.endLocation;
    const nextPickupLocation = pickupLocation != null ? sanitizeString(labelFromLocation(pickupLocation), MAX_LOCATION) : request.pickupLocation;
    const nextDropLocation = dropLocation != null ? sanitizeString(labelFromLocation(dropLocation), MAX_LOCATION) : request.dropLocation;
    ...
    let parsedStartLat = startLat != null ? parseNumber(startLat) : request.startLat;
    let parsedStartLng = startLng != null ? parseNumber(startLng) : request.startLng;
    let parsedEndLat = endLat != null ? parseNumber(endLat) : request.endLat;
    let parsedEndLng = endLng != null ? parseNumber(endLng) : request.endLng;
    const parsedCustomerLat = customerLat != null ? parseNumber(customerLat) : request.customerLat;
    const parsedCustomerLng = customerLng != null ? parseNumber(customerLng) : request.customerLng;
    const parsedRequestedAt = requestedAt ? new Date(requestedAt) : request.requestedAt ? new Date(request.requestedAt) : null;

    if (
      (!isValidLat(parsedStartLat) || !isValidLng(parsedStartLng) || isZeroCoordinate(parsedStartLat, parsedStartLng)) &&
      isValidLat(parsedCustomerLat) &&
      isValidLng(parsedCustomerLng) &&
      !isZeroCoordinate(parsedCustomerLat, parsedCustomerLng)
    ) {
    ...
    if (
      (!isValidLat(parsedEndLat) || !isValidLng(parsedEndLng) || isZeroCoordinate(parsedEndLat, parsedEndLng)) &&
      isValidLat(parsedStartLat) &&
      isValidLng(parsedStartLng)
    ) {
      parsedEndLat = parsedStartLat;
    ...
    if (!isValidLat(parsedStartLat) || !isValidLng(parsedStartLng) || !isValidLat(parsedEndLat) || !isValidLng(parsedEndLng)) {
      return res.status(400).json({ success: false, error: 'Unable to resolve route location. Type a clearer area or use current location.', code: 'MAP_COORDINATES_REQUIRED' });
    }

    if (parsedRequestedAt && Number.isNaN(parsedRequestedAt.getTime())) {
      return res.status(400).json({ success: false, error: 'requestedAt is invalid', code: 'INVALID_REQUESTED_AT' });
    }
    if (parsedRequestedAt && parsedRequestedAt <= new Date()) {
      return res.status(400).json({ success: false, error: 'requestedAt must be in the future', code: 'PAST_REQUESTED_AT' });
    }
    ...
    const updatePayload = {
      ...
      pickupLocation: nextPickupLocation || nextStartLocation,
      dropLocation: nextDropLocation || nextEndLocation,
      startLat: parsedStartLat,
      startLng: parsedStartLng,
      endLat: parsedEndLat,
      endLng: parsedEndLng,
      customerLat: parsedCustomerLat,
      customerLng: parsedCustomerLng,
      city: city != null ? String(city).trim() : request.city,
    ...
    if (vehicleType != null) {
      ...
      updatePayload.vehicleType = normalizedVehicleType;
    }
    ...
    if (desiredFare != null) {
      const parsedDesiredFare = parseNumber(desiredFare);
      if (!parsedDesiredFare || parsedDesiredFare <= 0) {
        return res.status(400).json({ success: false, error: 'desiredFare must be greater than 0', code: 'INVALID_FARE' });
      }
      if (parsedDesiredFare < MIN_FARE) {
        return res.status(400).json({ success: false, error: 'Minimum fare is Rs 50', code: 'FARE_TOO_LOW' });
      }
      updatePayload.desiredFare = parsedDesiredFare;
    }

    if (parsedRequestedAt) {
      updatePayload.requestedAt = parsedRequestedAt.toISOString();
    }

    await requestRef.update(updatePayload);
    const updatedDoc = await requestRef.get();
    return res.json({ success: true, request: serializeCustomerRequest({ id: updatedDoc.id, ...updatedDoc.data() }) });
  } catch (err) {
    return res.status(500).json({ success: false, error: err.message, code: 'UPDATE_CUSTOMER_REQUEST_ERROR' });
  }
};
